...
### バージョン情報
- **アプリケーション名**: Gemini Chatbot with Live2D (Natori Edition) - 管理画面感情コントロール
- **バージョン**: 3.3.0 (Admin Emotion Control)
- **作成日**: 2025年6月2日
- **最終更新**: 2025年6月11日（管理画面感情コントロールシステム実装完了）
- **動作確認済み**: 2025年6月11日（フロントエンド感情ボタン削除・Live2D表示拡張・管理画面感情制御実装成功）
- **テスト状況**: ✅ 統合起動システム、✅ 安全な停止機能、✅ プロセス管理、✅ 自動依存関係管理、✅ 管理画面感情コントロール
...
#### 3.6 Live2D感情コントロール（NEW）
- **WebSocket制御**: 管理画面からフロントエンドのLive2D表情をリアルタイム制御
- **美しいUI**: グラデーション感情ボタンとリアルタイム状態表示
- **対応表情**: 
  - 😊 笑顔 (Smile) - 黄色～オレンジグラデーション
  - 😮 驚き (Surprised) - 紫～青グラデーション
  - 😢 悲しみ (Sad) - 青～深青グラデーション
  - 😠 怒り (Angry) - 赤～深赤グラデーション
  - 😐 通常 (Normal) - グレー～黒グラデーション
- **リアルタイムフィードバック**: 
  - 現在の表情状態表示
  - ボタンアクティブ状態管理
  - 実行結果の詳細表示
- **エラーハンドリング**: WebSocket接続失敗時の適切なフィードバック
- **セキュリティ**: 認証済み管理画面のみからのコマンド送信

#### 3.7 WebSocketデバッグ機能（UPDATED）
...
#### 2.2 表情制御（管理画面統合版）
- **制御方式**: 管理画面からのWebSocket経由リモートコントロール
- **対応表情**: 5つの表情切り替え
  - 😊 笑顔 (Smile)
  - 😮 驚き (Surprised)
  - 😢 悲しみ (Sad)
...
- **管理画面UI**: 美しいグラデーションボタンとリアルタイム状態表示
- **フロントエンド**: 感情ボタン削除により、Live2D表示エリア拡張
...
- **Live2Dコンテナ**: 480x650px、左側固定配置（拡張版）
- **チャットコンテナ**: 右側可変幅、最小400px
- **レスポンシブ対応**: 1024px以下で縦並び配置に変更

#### 3.2 Live2D表示エリア（拡張版）
- **背景色**: #2a2a2a（ダークグレー）
- **キャンバス**: 440x580px、枠線と角丸スタイル（拡張サイズ）
- **感情制御**: 管理画面からのWebSocket経由リモートコントロール
...
**統合システムの特徴:**
- **ワンコマンド起動**: `npm start`でバックエンド・フロントエンド・管理画面が連動起動
- **自動依存関係チェック**: 初回起動時の自動`npm install`
- **ポート競合解決**: 既存プロセスの自動停止・再起動
- **起動確認**: 各サーバーの正常起動チェック（ポート監視）
- **プロセス管理**: PIDファイルによる確実なプロセス管理
- **ログ分離**: サーバー別ログファイル（`logs/`ディレクトリ）
- **安全な停止**: SIGTERM → SIGKILL による段階的停止
- **クリーンアップ**: 一時ファイルの自動削除

**開発モードの追加機能:**
- **ファイル監視**: バックエンドファイル変更時の自動再起動（nodemon）
- **リアルタイムログ**: コンソールでのリアルタイムログ表示
- **詳細デバッグ**: 各サーバーのデバッグ情報表示
...
| **chalk v4** | 安定したターミナル色付き表示、統合起動システム |
| **child_process** | Node.js標準、堅牢なプロセス制御 |
...
| 項目 | 仕様 |
|------|------|
| **起動コマンド** | `npm start` (本番) / `npm run dev` (開発) |
| **停止コマンド** | `npm stop` |
| **プロセス管理** | PIDファイル (`.pids`) による管理 |
| **ログ管理** | `logs/` ディレクトリ分離保存 |
| **ポート管理** | 自動競合解決、起動確認 |
| **依存関係管理** | 自動チェック・インストール |
| **エラーハンドリング** | 段階的停止、完全クリーンアップ |
